...
bool draw_camera_fit_toggle_overlay(bool fit_to_pane) {
  ...
  draw_list->AddRectFilled(rect.Min, rect.Max, color_u32(theme::white, hovered ? 234 : 214), 4.0f);
  draw_list->AddRect(rect.Min, rect.Max, color_u32(theme::camera_border), 4.0f, 0, 1.0f);
  const ImRect box(ImVec2(rect.Min.x + 6.0f, rect.Min.y + 4.0f), ImVec2(rect.Min.x + 18.0f, rect.Min.y + 16.0f));
  draw_list->AddRect(box.Min, box.Max, color_u32(theme::camera_box), 2.0f, 0, 1.0f);
  if (fit_to_pane) {
    draw_list->AddLine(ImVec2(box.Min.x + 2.5f, box.Min.y + 6.5f), ImVec2(box.Min.x + 5.5f, box.Max.y - 2.5f), color_u32(theme::accent), 1.8f);
    draw_list->AddLine(ImVec2(box.Min.x + 5.5f, box.Max.y - 2.5f), ImVec2(box.Max.x - 2.5f, box.Min.y + 2.5f), color_u32(theme::accent), 1.8f);
  }
  draw_list->AddText(ImVec2(box.Max.x + 6.0f, rect.Min.y + 3.0f), color_u32(theme::icon_strong), "Fit");
